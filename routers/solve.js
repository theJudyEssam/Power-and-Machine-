...
S_Router.post("/transformer/efficiency", (req, res)=>{
    ...
    let { R1, R2, X1, X2, Rc, Xm, a, V2, Pout, PF, options } = req.body;
    ...
    const I2_angle = leadlag * Math.acos(0.8); 
   ...
   // console.log(`V1_mag = ${V1_polar.magnitude} V1_angle = ${V1_polar.phaseAngle * (180 / Math.PI)}`)
    ...
    console.log(`Rc_complex is ${Rc_complex}`);
    const Ic = math.divide(V1_complex , Rc_complex); //complex num
    const Ic_polar = rectangularToPolar(Ic.re, Ic.im);

    //console.log(Pout);
    console.log(`Ic_polar.magnitude: ${Ic_polar.magnitude}`);
    console.log(`Rc: ${Rc}`);
    console.log(`V1 is ${V1_polar.magnitude}`)
    //console.log(req1);
